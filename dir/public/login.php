...
try {
    ...
    $carreras_agrupadas = [];
    $empresas = [];
    $db_error = "No fue posible cargar los datos del formulario porque la base de datos no está disponible.";
}
                ...
                <div class="form-slider" id="formSlider">
                    <form id="loginForm" action="../src/login.php" method="POST" class="form-pane active">
                        ...
                        <?php if (!empty($db_error)): ?>
                            <div style="background-color: #fff4cc; color: #8a6d3b; padding: 10px; border-radius: 5px; margin-bottom: 15px; text-align: center; font-weight: bold;">
    ...
    const htmlCarreras = `
        <div class="field dynamic-field" style="margin-top: 10px; animation-delay: 0.3s;">
            <select name="id_carrera" class="custom-select" required>
                ...
                <?php foreach ($carreras_agrupadas as $uni => $lista_carreras): ?>
                    <optgroup label="<?= htmlspecialchars($uni) ?>">
                        <?php foreach ($lista_carreras as $c): ?>
                            <option value="<?= $c["id_career"] ?>"><?= htmlspecialchars($c["carrera"]) ?></option>
                        <?php endforeach; ?>
                    </optgroup>
                <?php endforeach; ?>
            </select>
        ...
        switch (this.value) {
            case 'estudiante':
                fieldsHTML = `<div class="field dynamic-field" style="margin-top: 10px; animation-delay: 0.1s;"><input type="number" name="rut" placeholder="RUT (Sin puntos ni guion)" required></div>
                              <div class="field dynamic-field" style="margin-top: 10px; animation-delay: 0.2s;"><input type="text" name="cod_invitacion" placeholder="Código de invitación del Encargado" required></div>` + htmlCarreras;
                break;
            case 'encargado':
                fieldsHTML = `<div class="field dynamic-field" style="margin-top: 10px; animation-delay: 0.1s;"><input type="text" name="telefono" placeholder="Teléfono de contacto" required></div>
                              <div class="field dynamic-field" style="margin-top: 10px; animation-delay: 0.2s;"><input type="text" name="cod_invitacion" placeholder="Código de invitación del Director" required></div>` + htmlCarreras;
                break;
            case 'director':
                fieldsHTML = `<div class="field dynamic-field" style="margin-top: 10px; animation-delay: 0.1s;"><input type="text" name="telefono" placeholder="Teléfono de contacto" required></div>` + htmlCarreras;
                break;
            case 'tutor':
                fieldsHTML = `<div class="field dynamic-field" style="margin-top: 10px; animation-delay: 0.1s;"><input type="text" name="cargo" placeholder="Tu Cargo en la Empresa" required></div>
                              <div class="field dynamic-field" style="margin-top: 10px; animation-delay: 0.2s;"><input type="text" name="telefono" placeholder="Teléfono" required></div>` + htmlEmpresas;
                break;
        }
